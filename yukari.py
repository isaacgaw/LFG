...
Latent Acronyms:
    ...
    AA4: rps(poison surge), rjs(jammer surge), rdv(vdp), raa(attribute absorb), rls(lead swap)
...
async def on_message(message): # manually implementing the chuchu commands where the input is a string with whitespace, so you don't need to surround it with quotes.
    ...
    if message.content.startswith('-yukari'):
    ...
    if message.content.startswith('-123523141242425'):
        try: 
            for x in range(3461, 4001): 
                print(x)
                download_portrait(x)
                print('Done')
                print('')
    ...
    if message.content.startswith('-buildhelp'):
        print(message.guild,":", message.channel)
        print(message.author)
        print(message.content)
        print('')
